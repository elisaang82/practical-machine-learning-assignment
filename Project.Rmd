...
```{r}
newtrain <- subset( newtrain, select = -c(X,user_name, raw_timestamp_part_1, raw_timestamp_part_2,cvtd_timestamp ))
newtest <- subset( newtest, select = -c(X,user_name, raw_timestamp_part_1, raw_timestamp_part_2,cvtd_timestamp ))
```
...
newtrain <- subset(newtrain, select = -NAEval$colid[NAEval$percentageNA>0.97])
newtest <- subset(newtest, select = -NAEval$colid[NAEval$percentageNA>0.97])
```
...
## Preprocess the data
Center, scale and impute values.
```{r}
train_predictors <- subset(newtrain, select = -c(classe))
train_predicted <- newtrain$classe
preProc <- preProcess(train_predictors, method= c("center", "scale", "medianImpute"))
train_transformed <- predict(preProc, train_predictors)
test_transformed <- predict(preProc, newtest)
train <- data.frame(classe = train_predicted, train_predictors)
```
...
### Train
```{r}
rf_fit <- train(classe ~ ., data = train.training, method = "rf" )
```
...
```{r}
test.results <- predict(rf_fit, test_transformed)
```
